                    ...
                    if (clone)
                    {
                        ...
                        else
                        {
                            ...
                            if (rb)
                            {
                                ...
                                rb.linearVelocity = new Vector3(Mathf.Cos(angle), Mathf.Sin(angle)) * _RandomFloat(speed);
                            }
                        }
                    }
            ...
            else if (mode == Mode.Direction)
            {
                if (directionSettings.smartSpawn == true && ObjectCounter.objects > 0 || directionSettings.smartSpawn == false)
                {
                    ...
                    if (clone)
                    {
                        ...
                        {
                            var rb = clone.GetComponent<Rigidbody2D>();
                            if (rb) rb.linearVelocity = clone.transform.rotation * new Vector3(_RandomFloat(speed), 0, 0);
